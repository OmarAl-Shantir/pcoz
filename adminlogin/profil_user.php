...
if($per == 3 || $_SESSION['admin_logged']==$_GET['id']){
  if(isset($_POST['save'])){
    $userC->update_user_data($_GET['id'],$_POST['fullname'], $_POST['username'], $_POST['email'], $_POST['directory'], $_POST['active_input']);
    header("Refresh:0");
    die();
  }
  ...
  if(isset($_POST['password_change'])){
    if($_POST['new_pass'] == $_POST['new_pass_2']){
      $res = $userC->change_password($_GET['id'], $_POST['old_pass'], $_POST['new_pass'], 1);
...
<div class="card shadow mb-4">
  ...
  <div class="card-body">
    <form method="post">
      ...
      <div class="form-group row" id="prihl_meno">
        <label for="username" class="col-sm-2 col-form-label">Prihlasovacie meno:</label>
        <div class="col-sm-10">
          <input type="text" name="username" class="form-control bg-light border-1" id="username_input" value="<?php echo (isset($_POST['username']))?$_POST['username']:$data['username']?>">
        </div>
      </div>
      ...
      <div class="form-group row" id="adresar">
        <label for="directory" class="col-sm-2 col-form-label">Adresár:</label>
        <div class="col-sm-10">
          <input type="text" name="directory" class="form-control bg-light border-1" id="directory_input" value="<?php echo (isset($_POST['directory']))?$_POST['directory']:$data['directory']?>">
        </div>
      </div>
